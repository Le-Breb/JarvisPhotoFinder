...
export default function SimilarityGraphPage() {
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    // This allows dialogs to work properly
  ...
  useEffect(() => {
    ...
    node.filter(d => !!(d.representative_face && d.representative_bbox)) // Only add if both face and bbox exist
      .append('image')
      .attr('href', d => {
        const filename = d.representative_face?.replace('images/', '') || '';
        const bbox = d.representative_bbox?.join(',') || '';
        return `/api/face/${filename}?bbox=${bbox}&padding=30`;
      })
    ...
    // Photo count badge - size scales with the number
    node.append('circle')
      .attr('r', d => {
        // Scale badge size: smaller and more subtle
        const numDigits = d.photo_count.toString().length;
    ...
    node.append('text')
      ...
      .attr('y', d => -(Math.sqrt(d.photo_count) * 0.5 + 2) + 1)
      .attr('dominant-baseline', 'middle')
      .attr('text-anchor', 'middle')
      .attr('font-size', d => {
        // Scale font size: very small for subtlety
        const numDigits = d.photo_count.toString().length;
  ...
  return (
    <>
      ...
      {/* Photo Dialog - outside main container so it works in fullscreen */}
